...
// camera
gps::Camera myCamera(
    glm::vec3(48.798889, 101.910858, 157.128387),
...
void processMovement() {
    ...
    if (pressedKeys[GLFW_KEY_V]) {
        myBasicShader.useShaderProgram();
        glUniform1i(FogViewLoc, 1);
    }


    if (pressedKeys[GLFW_KEY_B]) {
        myBasicShader.useShaderProgram();
        glUniform1i(FogViewLoc, 0);
    }

    if (pressedKeys[GLFW_KEY_F])
    {
        glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);//poligonal
    }

    if (pressedKeys[GLFW_KEY_G])
    {
        glPolygonMode(GL_FRONT_AND_BACK, GL_POINT);//smooth
    }
...
void initModels() {
    ...
    ground.LoadModel("models/MAPA2/mapa5.obj");
...
void initUniforms() {
    ...
    //set light color
    lightColor = glm::vec3(0.4f, 0.4f, 0.4f); //white light
    ...
    myBasicShader.useShaderProgram();
    FogViewLoc = glGetUniformLocation(myBasicShader.shaderProgram, "fogView");
}
